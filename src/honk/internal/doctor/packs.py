...
class GlobalPack:
    ...
    def run(self, plan: bool = False) -> PackResult:
        ...
        checks.append(
            PackCheck(name="os", passed=True, message=f"Operating system: {os_name}")
        )

        # Check architecture
        arch = platform.machine()
        checks.append(
            PackCheck(name="arch", passed=True, message=f"Architecture: {arch}")
        )
        ...
        try:
            ...
            checks.append(
                PackCheck(
                    ...
                    remedy=f"Ensure {tmp_dir} exists and is writable"
                    if not writable
                    else None,
                )
            )
